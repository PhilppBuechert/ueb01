...
function fillTheader($colArray) {
	
	if(isset($colArray) && is_array($colArray)) {
		
	    foreach ($colArray as $colItem) {
	        echo '<th scope="col">' . $colItem . ':</th>';
	    }
...
function fillTbody($arrayOfArray) {

    if(isset($arrayOfArray) && is_array($arrayOfArray)) {
	    ...
	    foreach ($arrayOfArray as $array) {
		    ...
		    if (isset($array) && is_array($array)) {
			    ...
			    foreach ($array as $i => $value) {
			    	...
			    	if ($i == "last") echo '<td>' . $value . actionButtons() . '</td>';
